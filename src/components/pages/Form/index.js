...
class Form extends Component {
  ...
  getState(field) {
    const postcodeSuccess =
      (field === 'postcode' && isValidPostcode(this.state[field])) ||
      field !== 'postcode';
    const postcodeDanger =
      field === 'postcode' && !isValidPostcode(this.state[field]);

    if (this.state[field] !== '' && postcodeSuccess) return 'success';

    if (this.state.isValidating && (this.state[field] === '' || postcodeDanger))
      return 'danger';
    ...
    if (this.isValid()) {
      ...
      const materialToSend =
        filteredMaterials.length === 1
          ? filteredMaterials[0]
          : find(materials, { id: Number(material) });
  ...
  isValid() {
    ...
    return (
      ...
      (material !== '' || materialSelection.length === 1)
    );
  }
  ...
  render() {
    ...
    const i18nPlaceholder =
      placeholder || intlPlaceholder || 'Enter a postcode...';

    return (
      <Container>
        <form method="GET" action="" onSubmit={this.handleSubmit}>
          <Title as="h2">
            <Text id="form.title">What can you recycle at home?</Text>
          </Title>
          ...
          <Grid>
            {!prefilledPostcode ? (
              <Grid.Item style={{ flexBasis: '200px' }}>
                <FormGroup>
                  <FormGroup.Label for="postcode">
                    <Text id="form.postcode.label">Postcode</Text>
                  </FormGroup.Label>
                  <FormGroup.Control>
                    <Input
                      type="text"
                  ...
                  {isValidating && !isValidPostcode(postcode) ? (
                    <FormGroup.Help>
                      <Text id="form.postcode.validation">
            ...
            {materialSelection.length !== 1 ? (
              <Grid.Item style={{ flexBasis: '300px' }}>
                <FormGroup>
                  <FormGroup.Label for="material">
                    <Text id="form.material.label">Material</Text>
                  </FormGroup.Label>
                  <FormGroup.Control>
                    <Select
                      id="material"
                    ...
                    >
                      <option value="">
                        <Text id="form.material.placeholder">
                          Select material
                        </Text>
                      </option>
                  ...
                  {isValidating && material === '' ? (
                    <FormGroup.Help>
                      <Text id="form.material.validation">
                        Choose a material to check
                      </Text>
                    </FormGroup.Help>
                  ) : null}
                </FormGroup>
            ...
            <Grid.Item>
              <FormGroup>
                <FormGroup.Control>
                  <Button type="submit">
                    {button || <Text id="form.button">Submit</Text>}
                  </Button>
