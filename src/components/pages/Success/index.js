...
export default class Success extends Component {
  ...
  componentDidMount() {
    const { loadRoute, locale, postcode, material, token, apihost } =
      this.props;
  ...
  render() {
      ...
      return (
        <Container>
          <Back onClick={() => loadRoute('form')}>
            <Text id="success.back">Search again</Text>
          </Back>
          <Title as="h2" state="success">
            <Text id="success.kerbside.title">Good news!</Text>
          </Title>
          <p>
            <Text
              id="success.kerbside.message"
              fields={{ material: material.name, postcode }}
            >
              <span>
                You can put {material.name} in your home recycling in {postcode}
                .
              </span>
            </Text>
          </p>
          ...
          <p>
            <Text id="success.more">Find out how to recycle more items at</Text>{' '}
            <OutLink brand={brand} />
          </p>
    ...
    const shuffledAvailable = data.available_materials.sort(
      () => 0.5 - Math.random()
    );
    ...
    return (
      <Container>
        ...
        <Title as="h2" state="info">
          <Text id="success.no_kerbside.title">Unfortunately...</Text>
        </Title>

        <p>
          <MarkupText
            id="success.no_kerbside.message"
            fields={{
              postcode,
          ...
          >
            You <strong>can&apos;t</strong> put {material.name} in your home
            recycling in {postcode} at the moment, you&apos;ll need to take them
            to your
          </MarkupText>{' '}
          <NearestRecycling
            brand={brand}
